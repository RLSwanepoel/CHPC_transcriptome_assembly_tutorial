...
sub cleanid{ my $d=$_[0]; $d=~s/\W/_/g; $d; } # ugh, prfix:id problems
...
sub putgff {
  ...
  # 2015 add MININTRON check/close exons w/ tiny/no gap; splign min sep=1, begin = 1 + last end.
    ...
    # filter: drop or keep as 2nd set to check after 1st parts collection
    # in putgff() or processGeneSet() ?
		if($MINIDENT and $pident < $MINIDENT) {
		  my $xout= "#px.".join("\t",@$x)."\n"; 
			push @xonpoor, $xout; 
			next; # xon
		}
	  ...
	  $maln+= $aln; $mlen+= $len;
		...
		$mident += $pident * $aln;   # pident = 0..1 range
  ...
  my $msplscoresave= $msplscore;
	...
	my $pctident= int(0.5 + 1000 * $mident/$maln)/10;
...
=cut
	...
	if($trattr =~ /offs=(\d+).(\d+)/) { # add cds-exons : option?  check $trattr=~/aalen=/ also ?
	  my($cb,$ce)=($1,$2); 
	  my $cdir= ($mor eq '-')?-1:1;
	  @cdsgff= makeCDSexons($cb,$ce,($cdir == -1),\@xon);
# 	  foreach my $x (@xon) {
# 	    my @c= @$x;
...
sub makeCDSexons {
  my($cb,$ce,$revor,$xons)= @_;
	my @cdsgff=();
	# if($trattr =~ /offs=(\d+).(\d+)/) 
	# my $revor= ($mor eq '-')?1:0;
	if($cb and $ce > $cb and ref($xons)) { # add cds-exons : option?  check $trattr=~/aalen=/ also ?
	  foreach my $x (@$xons) {
	    my @c; # my @c= @$x;
	    if(ref($x) =~ /ARRAY/) { @c= @$x; }
	    ...
	    if($te > $cb and $tb < $ce) {
	      ...
	      ## fixme again; -strand needs c3,c4 swap also
        if($tb < $cb) { my $d=$cb-$tb; 
          if($revor) { $c[4] -= $d; } else { $c[3] += $d; } # FIXME: - bad or offset; flip d sign, $or * $d
          }
        if($te > $ce) { my $d=$te-$ce; 
          if($revor) { $c[3] += $d; } else { $c[4] -= $d; }
          }
	      $c[5]=1; # $c[7]= $phase; ## FIXME: need phase calcs
	      $c[8] =~ s/;Target=.*//;
	  		my $xout= join("\t",@c)."\n"; 
	  		push @cdsgff, $xout;
	    }
...
sub relocateGenePart {
  ...
  if($maln2 != $maln) {
    ...
    my $pcta2= int(0.5 + 100 * $maln2/$mlen);    
    $mrna[8]=~s/cov=$pcov%,$maln/cov=$pcta2%,$maln2/;
  }
   
  my($cb,$ce)= $mrna =~ /\boffs=(\d+).(\d+)/;
  my($revor)= ($mrna[6] eq '-')?-1:0;
  my @cdsnew= makeCDSexons($cb,$ce,$revor,$xkeep);
