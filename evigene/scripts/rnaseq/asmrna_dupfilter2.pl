...
my $debug= 0;
use constant VERSION => '2013.09.07'; # 01'; # 08.09'; #08.07; 03.25; 24; # '2013.02.21';
...
use constant EQGENE_CHANGES_NOALN => 1;
...
###  sort ^Qclen, ^Align, vTtlen: cat $atab | grep -v '^Qid' | sort -k2,2nr -k7,7nr -k6,6n 
  ...
  while(<$inh>) { # maybe local table, sorted, or from file
    ...
    if($tc > $qc){ ($qd,$td)=($td,$qd); ($qc,$qw,$tc,$tw)=($tc,$tw,$qc,$qw); } # swap to greater cds?
		...
		if(EQGENE_CHANGES_NOALN && $lastd && $lastd ne $qd) { # check for $ltd missing eqgene 
		  #* Need to change this altmap class: NO cds-align b/n td,qd does mean something, poor mapping paralogs end up here
			...
			#m2t: ERR: trclass inline:Funhe2Exy3m149508t1_G2        drop    altmap  Funhe2Eq7m074376t1      99/35           0,0,pflag:3
			## ^^ fixme remove dupl map ids, best from eqgene table.
			...
			## check each lastd/@ted for ismain{id}, class{id}, and aasize{id} : pick one to keep as main
			  ...
			  if(0) {
			  my $taw= $aasize{$ted[0]} || 0;  my $tcl= $class{$ted[0]}||"noclass";
			  my $ldw= $aasize{$lastd} || 0;   my $lcl= $class{$lastd}||"noclass";
			  if($taw > $ldw) { # swap for new main ?
				...
				foreach my $te (@ted) { unless($havepair{$lastd}{$te}) { 
					next if($te =~ /_G\d+$/); # dup map id syntax
	        ...
	        # my $taw= $aasize{$te} || 0;
					...
					if($palmap) {
						$miss++; 
    				my $pidalnval="99/$palmap";  # .$fakeantiflag ??
...
## 13sep06 : maybe not here .. trust align score if it exists for pair? at least needs flag if changes pal.
		...
		if(EQGENE_OVERRIDES_ALN && $neqgene>0) { 
...
unless(TEST3) {
    ...
    if($pal < $TINYALN) { } #defer: $aclass="noalign$isfrag";  
      ...
      if($pid >= 99) {
        #if($samealn) {  $aclass .= "2"; }   # bad calls here, pal lowish 80%
        #elsif
        if($pal >= $PHIALN or $tinyalndiff) { $aclass .= "1"; }  # "100"; use or not? PHIALN reused: was 99 here
      }
