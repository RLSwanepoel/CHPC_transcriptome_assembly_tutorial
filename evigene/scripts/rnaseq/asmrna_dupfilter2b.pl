...
use FindBin;
use lib ("$FindBin::Bin/../"); # 201405 add; assume evigene/scripts/ layout: main, prot/, rnaseq/, ...
...
my $debug= 0;
use constant VERSION => '2016.02.11'; # MINBASEOVER/TINYALN changes
...
$AAPART=$ENV{aapart}||100; # was 100; # for aapartial # 201402: THIS aapart cut is problem, drop.mains uniq orthologs here
      ...
      # including recover perfectfrag drops of complete/utrgood trasm for AAPART-bad drop.mains
      # part of problem that shorter-AA are sometimes best trasm
$AAMINBAD=$ENV{aaminbad}||90; #was 200;  # for utrbad class
...
my $OK_CDSUTR_default= 60;
my $BAD_CDSUTR_default= 30; # % CDS/trlen
$OK_CDSUTR= 0;
...
my(%aasize, %trsize, %aaqual, %aablast, %aablastref,$naabl); $naabl=0;
...
my $OUTH= *STDOUT;
unless($ENV{TMPDIR}) { if(my $cdir=FindBin::cwd2()) { $ENV{TMPDIR}=$cdir; } } ## FIXME sort TMPDIR, cant use /tmp
...
## FIXMEd for cds.qual not aa.qual, col2 == cds-size, col4=aasize,qual, col5=trsize
    ...
    while(<F>) { next if(/^#/ or /^total/); 
      ...
      if($aqual) { 
        my($aqlen)= $aqual=~m/(\d+)/; 
        if($aqlen < $alen and $aqlen*3 >= $alen-3) { $alen= int($alen/3); $iscds++; } #cdslen not alen
        elsif($aqlen == 0 and $tlen == 0 and $iscds) { $alen= int($alen/3);  } # datatable bug
...
sub readEqualGene {
  ...
  while(<$inh>) {  
    ...
    # new bleqgene mapqual: 85a,100i,2469l,4x,Spl:29%,KB668900 == %align,%ident,bases,exons,Spl:split
		$gmapqual{$mid}="$mapqual\t$loc"; # or $loc\t$mapqual
		
    if($altnotover) {
      my @nealt= split",",$altnotover;
      map{ $neqalts{$mid}{$_}=1; } @nealt;
    }
    ...
    $nids++; my $jov=0; 
		...
		foreach my $ov (@ovd) {
		  ...
		  $ovd =~ s/_C\d+$//;  ## what of split maps: id _C[12] ? skip or chop _C?
			...
			my($xeq)= ($cx=~s/xe(\d+)//)?$1:0; $nxeq++ if($xeq);
			...
			if($ca >= $MIN_EQGENE) { 
			  ...
			  $eqgenes{$mid}{$ovd}= $ca; # pct of mid aligned to ovd
			  ## $eqgenes{$ovd}{$mid}= $ca; ## NO, not reciprocal now; 
			  $eqexons{$mid}{$ovd}= $xeq; # new, test
			  $jov++; 
			  }
		}
		$nov++ if($jov);
...
use constant NOTPOOR => kAATINY + kAADUP + kAAUTRBAD + kAAGAPS; # - kAAUTRPOOR
  ...
  ## TEST1603: $pal =~ m/altmap\d+xeq/ ; xeq for althi1 means redundant == AADUP
  ...
  if($pal =~ m/altmap\d+xeq/ and $cla =~ /althi1|part/) {
    $ispoor |= kAADUP;  # add new bitflag kDUPEXONS ?
    ## ispoor conflicts here w/ refbest dups
    #$tbits.=",xeq:$xxx";
  }
