...
my $VERSION = "2011.09.02"; # new 2011.09
...
## no, cant get proper hsp-group == gene-align here
    ...
    $alignment_length -= $mismatches; # FIX: 100207 
...
sub printGeneLocations {
  ...
  assignBestgene(); # NEW; this could be buggy ?**
...
sub printOneLocation {
  ...
  foreach my $ex ((sort{ $$a[5] <=> $$b[5] } @$exons)) {
    ...
    $sum_ident += $pctident * $alignment_length;
    if(1) { # == GFF_LOC
  ...
  if($addgene) {
    ...
    $attr.= ";tlen=$tlen" if($tlen);
    $attr.= ";align=$sum_align;pident=$pident";
    unshift @gffout, join("\t",
