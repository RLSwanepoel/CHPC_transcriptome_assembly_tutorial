...
=cut
  ...
  sub puta { 
   ...
   ## arbm: change here bits > not highest score but median of orthogroup?
    ...
    if($USE_MEDBITS) {
      # ($n,$md,$mn,$sd,$min,$max)= medmeansd( $arbm{$ar}{$tg} );
      my($mdaa,$naa,$minaa,$maxaa)= median( $arbm{$ar}{$tg} );
      $v[$SCORECOL]= $mdaa if($naa>0);
      # $v[0]= $mdaa if($naa>0);
    }
   ...
   ## redo this size-outlier calc: use md, median size, and +/- 2stdev outlier?
    if(1) {   
        ...
        $md= @sl[int(@sl/2)]; 
        # ($max,$min)=  @sl[0,-1]; # << revise to use @sl[1,-2] or some not quite max,min
        ##no# $xl=($tl > 1.5*$max)?2:($tl < 0.65*$min)?-2:0; 
        $xl=($tl > 1.9*$md)?2:($tl < 0.65*$md)?-2:0; # maybe this, 1.5 or .66 outside median
    } else {   
...
#     $bt=$v[0]; $xd=$v[-1]; $d=$v[-2]; $arb{$ar}{$ts}=$bt; $ard{$ar}{$ts}=$d; 
#     $arn{$ar}++; $ts{$ts}++;  $arx{$ar}{$ts}=$xd if($xd); $tsx{$ts}{$xd}++; 

    # $bt=$v[0]; # OPTION: swap bitscore for ident/[1] or align/v[2]
    $bt=$v[$SCORECOL];
    $xd=$v[-1]; $d=$v[-2]; 
  ...
  my @cset= (@ts>1) ? (0,1) : (1); # Common not for 1 species tables
  for my $k (@cset) {
    my @aset= ($k==0) ? @ap : @ar;
    ...
    if($DOALL) {
    print join("\t","nGroup", $SCORELAB, qw( dSize rGroup nGene),"r".$SCORELAB,"outlierSize"),"\n";    
    } else {
    print join("\t","nGroup", $SCORELAB, qw( dSize nGene),"r".$SCORELAB,"outlierSize"),"\n";    
    }
...
#     foreach my $t (@ts) {  my($nt,$ab,$nr,$abr,$ad,@xl,$ng);
#       $nt=$sn{$t}||1; $ab=int(0.5+$decp*$sb{$t}/$nt)/$decp;  
#       $nr=$snt{$t}||1; $abr=int(0.5+$decp*$sb{$t}/$nr)/$decp;  $ad=int(0.5+$decp*$sd{$t}/$nr)/$decp; 
#       @xl= map{ my $xc=$sx{$t}{$_}; my $xp=int(1000*$xc/$nr)/10; $xlkey{$_}."=$sx{$t}{$_} ($xp%)" } sort keys %{$sx{$t}}; 
#       printf "%-9s: ",$t; 
#       # print "n=$nt; nreal=$nr; abits=$ab; dlen=$ad; arealbits=$abr; xtralen:@xl\n";  
#       print join("\t",$nt,$ab,$ad,$nr,$abr,@xl),"\n";
#     }
