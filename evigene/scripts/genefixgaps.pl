...
=item gapfix annots
    ...
    gapfix=b+29 ; gapfix=e-11; 
        EXON_LOC_IS_ADJUSTED ** input exon end has been adjusted now
...
sub testgene
{
  ...
  my $issplit= ($mrna->[8] =~ /splitgene/)?1:0;
  my ($genegap)= ($generow) ? $generow->[8] =~ /(gap=[^;\s]+)/ : "";
  # ^^ test this vs nfix
...
#     gapfix=b+29 ; gapfix=e-11; 
  ...
  unless($nfix) {    
    ...
    else { print "#warn: $gid nothing  fixed\n"; }
    return putgene( $generecIN, $geneother);  
  }
  elsif($nfix and not $genegap and $generow) { # ok mistake? missing gene gap= flag
