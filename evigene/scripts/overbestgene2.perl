...
use constant VERSION => '2013.09.29'; # 27
...
my $RESCORE_ONLY = 0;
my $VECSCORE=1; # no $PUT_SCORESUM = 0;
...
if($mustkeepdrop) {
  ...
  while(<F>){
    ...
    if($qual =~ /^(drop|skip|bad|0|false)/i) { $skipgene{$gid} = kSKIP_REQUIRED; }
    elsif($qual =~ /^(keep|best|good|1|true)/i) { $mustkeepgene{$gid} = kKEEP_REQUIRED; }
      ...
      foreach my $gref (@sgenes) {
        ...
        if($keepgene{$gid}) { # should not be here now
          ...
          foreach my $ex (@$rexons) {
            ...
            $nover++ if (overlap1($ex, \@didexons, $typeover, 0));
        ...
        if($DO_OVEREXON2 and $nover == 0 and $xbad == 0 and $typeover !~ /exon/) {
          ...
          my $typeover2="exon"; 
          my $cdsw = $gref->[jCDSLEN]; # "hidden" gref attributes
          ...
          foreach my $ex (@$rexons) {
            ...
            $nover++ if (overlap1($ex, \@didexons, $typeover2, oSTRANDED));
            push @overid1, @overids; # save for skip report
            $baseo  += $overbases; 
            $baselen+= $overlen;  
            }
...
sub putgene {
  ...
  foreach my $ex ($gref, @$exons) {
    ...
    my $pscore=$tscore; 
    unless($VECSCORE) { 
      #my($ss)= $tattr=~/scoresum=([^;\s]+)/;
      my $ss= $ex->[jSCORESUM] || 1; # only in mRNA/gref
      if($typ =~ /$mrnatypes/) { 
...
sub overlap1 {
  ...
  foreach my $rloc (@$locs) {
    ...
    next if($typeover and $lty !~ /$typeover/);
    my $over= ($tb <= $le && $te >= $lb) ? 1 : 0;
    $over= 0 if($flags & oSTRANDED and ! _eqstrand($lo,$to) ); ## $lo ne $to
    if($over and $pctover) {
      my ($bb,$be)= ( _max($tb,$lb), _min($te,$le) );
      my $maxo= 1+abs($be - $bb);
      my $leno= _min( abs($le - $lb), abs($te - $tb)) || 1;
      ...
      if(1) {
        ...
        if($sumgenescore) {
          ...
          foreach my $i (0..$#sc) { 
            ...
            if($i == $inexon) # 201208
            {  
              my $vintr= ( $hasKeyNINTRON ) ? $sc[$inexon] : $sc[$intr]; 
              $vintr =~ s,/.*,,; 
              my $vinexon= ( $hasKeyNINTRON ) ? $vintr : ($vintr > 0) ? 1 : ($vintr < 0) ? -1 : 0;
...
# }
      ...
      if($hasKeyNINTRON and $validin) { $validin= int(5*$validin)/10; }
