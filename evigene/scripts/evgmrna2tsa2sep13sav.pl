...
use constant VERSION => '2013.08.10'; # 06.21'; # 05.31; # 28; 20; 07; # 05; # '04.20'; # .16'; # 03.20
...
## FIXME4: utrorf buggered, ids not in okids or .mrna, but are okay in .aa,.cds
  ...
  my($ok,%main,%mainsize,%alt,%altdrops,%balt,%drop,$outh,$outpubidh,$inh);
  ## $ok= open($inh,$trclass);
  ...
  while(<$inh>) {
    ...
    unless($ok eq 'okay') { $drop{$td}=1; $cl=$ok.$cl; next unless($SHOWDROPS); } # OPTION: include drops?
    if($hasokids and not $okids->{$td}) { $drop{$td}=1;  $cl='dropid'.$cl; next unless($SHOWDROPS);  } # unless $td =~ /utrorf/ ??
    $cl=~s/a2$//;   
