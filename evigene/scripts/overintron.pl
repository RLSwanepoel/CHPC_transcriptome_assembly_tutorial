...
sub testgene
{
  ...
  # @exons= reverse @exons if($gstrand eq "-");
...
sub overlaps {
  ...
  if($dointrons) {
    ...
    if($do1) { 
      my $ovi=overintron($ref, $tb1, $te1, $to, 0); 
      $overin += $ov1 if($ovi>0); 
      $inerr += $ovi if($ovi<0); 
    }
    if($do2) { 
      my $ovi=overintron($ref, $tb2, $te2, $to, 0); 
      $overin += $ov2 if($ovi>0); 
      $inerr += $ovi if($ovi<0); 
    }
  }
  ...
  if($hasintrons) {
    my $ovi= overintron( $ref, $tb + 2*SPLICE, $te - 2*SPLICE, $to, kIntronInside); 
    if($ovi == kINTRONERROR_INSIDE) { $inerr += -1; }  # NOT -3 here
  }

#  if($otheroverlaptest) {
#     return ($ovbases,$overin,$inerr,$lid) if ($over);
#   }
  ...
  for (my $ib = $ib1; $ib <= $ib2; $ib++) {
    ...
    foreach my $rloc (@locs) {
      ...
      if($testtype == kIntronInside) { 
        # return kINTRONERROR_INSIDE if($tb <= $inb and $te >= $ine and $samestrand);
        if($ok==0) { $ok= kINTRONERROR_INSIDE if($tb <= $inb and $te >= $ine and $samestrand); }
      } else { 
        # if($tb <= $le && $te >= $lb) { return ($samestrand) ? 1 : -1; }
