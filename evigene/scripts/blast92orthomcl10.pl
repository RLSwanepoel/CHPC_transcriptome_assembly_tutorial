...
if( SUMSCORE ) {
    ...
    sumscore( \%bspans, $qid, $sid, $pctident, $alignment_length, $prob, $bit_score, $q_start, $q_end, $s_start, $s_end) 
      if($bit_score > 0.25 * $bmax or $bspans{$sid}); # should drop this filter?
...
sub processQuery {
  ...
  foreach my $sid (sort keys %$bspans) {
    ...
    if($minident &&  (100*$sumalign/_max($qlen,$slen)) < $minident){ $skips{minident}++; next; } # filterbpo.pl WEAKMAT => 0.40;
