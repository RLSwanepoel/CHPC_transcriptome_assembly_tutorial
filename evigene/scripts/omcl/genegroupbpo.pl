...
my %PLA9sppmap = ( # FIXME, used for occurrance
# plant9
...
my %FISH11sppshort = map{ $_ => ucfirst($_) } @FISH11spplist;
...
btag "similar_genes";  
  foreach my $gn1 (@$gna) {
    ...
    btag "similarity", 1;
    ptag "spp",$sna,1;
    ptag "acc",($SPPGN?$gn1:$gn),1;
    ptag "eval",$ev,1;
    ptag "iden",$pi,1;
    ptag "def",$gdef,1 if($gdef);
    ptag "dbx",$gdbx,1 if($gdbx);
    etag "similarity", 1;
  }
etag "similar_genes"; 
...
## DAMIT: names have spp:id; here need species_id, what else ??
  ...
  ## DAMIT: make spp:id same as omcl spp_id; other dingbats here?
  $g1= fix_sppid($g1); # my($spp1,$g1a)= split /[:_]/, $g1, 2; $g1=$spp1.'_'.$g1a if($g1a); # or : ??
  $g2= fix_sppid($g2); # my($spp2,$g2a)= split /[:_]/, $g2, 2; $g2=$spp2.'_'.$g2a if($g2a); # or : ??
