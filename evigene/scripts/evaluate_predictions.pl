  ...
  foreach my $pred ( @predlist ) {
    ...
    while(<G>) {  
      ...
      if(/\tmRNA/) {
        ...
        ($clen,$xlen)= m/;cxlen=(\d+).(\d+)/;  # fixme if missing
        ($aalen)=m/;aalen=(\d+)/;  #? aalen=nnn,pctaa?
        my ($aaseq)= m/;protein=([^;\s]+)/;
        ...
        $ispartialaa= -1;        
        if(  $aaseq ) {  # dont count twice? isncrna
      ...
      if( $atend or (/\tmRNA/ and $lgid and $gid ne $lgid) 
        and not $isalt #??
