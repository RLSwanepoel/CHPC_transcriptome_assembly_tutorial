...
die "usage: bestupdate -action=xxx -conf=evigene.conf  in.gff ...
  ...
  unless($optok and $action and -f $ingff);
...
sub openio {
  my($ingff, $outgff)= @_;
  my($inh, $outh);
  open( $inh, $ingff) or die "ERR: read $ingff";
  open( $outh,">$outgff") or die "ERR: write $outgff";
  return($inh, $outh);
}
...
sub sortoutchr 
{
  ...
  {
    my($gchr,$gchr1st,$gb,$ge,$gsc)=(0) x 10;
    my $gene= undef;
    my @generec=();
  ...
  # FIXME: gene for splitgene; same mRNA ID diff scaffolds: need new gene rec for each?
    foreach my $mid (@mid) {
      ...
      unless($gchr) { $gchr1st= $gchr= $mrna->[0]; $gb= $mrna->[3]; $ge= $mrna->[4]; }
      if($gchr ne $mrna->[0]) { # only rare splitgene ?
      ...
      if($addgene){ $mrna->[8] =~ s/;gene=[^;\s]+//; # BUT remove old**
                  $mrna->[8] =~ s/;/;gene=$gid;/; }
      } # @mrna splitgene loop
        
      push (@generec, @feat); # add mrna record
    }
    
    if($gene) {
      $gene->[3]= $gb; $gene->[4]= $ge; $gene->[5]= $gsc;  
      #above.beforeft# unshift( @generec, $gene) if($addgene);
    }
    if(@generec) {  
      my $chr = $gchr1st; # $gene->[0]; # or use $atchr param? 
      if($chrsortgene and $gchr1st ne $gchr) {  
...
sub genesort_update 
{
  ...
  my ($pubid, $predid, $gid, $pubidnum, $ngene,$nmrna,$nexon,$nother)= (0) x 10;
  my (%pubidmap, %geneidmap, %featlist, %commlist); # include alt-tr ?
 
  my $addgene = $public_options{'addgene'} || ""; # i.e. add gene record enclosing mRNA (for alttr)
...
use constant DEBUG_SORT => 0;  
  ...
  my ($altcomm, $isalt, $lchr, $comsave)= (0) x 10;
  $pubid= "0header";
  ...
  while(<$inh>) {
    ...
    if(/^\w/) {
      ...
      if( $col[2] eq "gene" ) { 
        ...
        $pubidmap{$predid}= $pubid; # ok for same. 
        $pubidmap{$pubid}= $predid;
        $nmrna++; 
